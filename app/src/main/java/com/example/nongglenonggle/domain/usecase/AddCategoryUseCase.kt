...
class AddCategoryUseCase @Inject constructor(private val firestoreSetRepository: FirestoreSetRepository) {
    suspend operator fun invoke(name:String,docRef:DocumentReference, id:String){
        return firestoreSetRepository.addNoticeToCategory(name,docRef, id)
    }
}